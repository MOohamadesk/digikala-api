...
async function MenuFetch() {
  try {
    const response = await fetch("http://localhost:3000/data-01");
...
async function fetchStories() {
  try {
    const response = await fetch("http://localhost:3000/data-02");
    const data = await response.json();
...
async function viewStory(storyId) {
  try {
    await fetch(`http://localhost:3000/data-02/stories/${storyId}`, {
      method: "PATCH",
      headers: {
...
async function fetchFullSlider() {
  try {
    let response = await fetch("http://localhost:3000/data-03");
    if (!response.ok) {
...
async function fetchSumLinks() {
  try {
    const response = await fetch("http://localhost:3000/data-04");
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    const data = await response.json();
    ...
    document.getElementById("sumLinks").innerHTML = deepLinks + moreButtonHtml;

    document
      .querySelector(".more-modal-button")
...
function populateModal() {
  ...
  document.getElementById("modalTopIcons").innerHTML = topIconsHTML;
  document.getElementById("modalCards").innerHTML = cardsHTML;
}

document.addEventListener("DOMContentLoaded", fetchSumLinks);

let amazingSwiper = null;

async function fetchAmazing() {
  try {
    let res = await fetch("http://localhost:3000/data-05");
...
function createProductSlide(product, index) {
  ...
  if (rrpPrice > 0 && sellingPrice > 0 && rrpPrice > sellingPrice) {
    discountPercent = Math.round(((rrpPrice - sellingPrice) / rrpPrice) * 100);
  }
      ...
      <div class="flex flex-col items-center justify-center w-full ">
        ...
        <div class="w-full flex flex-row items-center justify-between mb-1">
          ...
          <div class="flex items-center justify-end gap-1 text-neutral-700">
            <span class="text-lg font-bold">${formatPrice(sellingPrice)}</span>
...
async function fetchBanners() {
  try {
    const response = await fetch("http://localhost:3000/data-06");
    const data = await response.json();
...
async function fetchCategories() {
  try {
    const response = await fetch("http://localhost:3000/data-07");
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
...
document.addEventListener("DOMContentLoaded", function () {
  fetch("http://localhost:3000/data-08")
    .then((response) => {
      if (!response.ok) {
      ...
      if (container) {
        container.innerHTML = `
          <div class="w-full text-center py-8">
            <p class="text-red-500">خطا در بارگذاری بنرها</p>
            <button onclick="location.reload()" class="mt-2 px-4 py-2 bg-blue-500 text-white rounded-lg">
              تلاش مجدد
            </button>
          </div>
        `;
...
async function fetchPopularBrands() {
  try {
    const response = await fetch("http://localhost:3000/data-09");
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
...
async function loadDoubleSubBanners() {
  try {
    const response = await fetch(`http://localhost:3000/data-10`);
...
document.addEventListener("DOMContentLoaded", function () {
  setTimeout(loadDoubleSubBanners, 100);
});

if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", loadDoubleSubBanners);
} else {
  loadDoubleSubBanners();
}

document.addEventListener("DOMContentLoaded", function () {
  fetch("http://localhost:3000/data-11")
    .then((response) => {
      if (!response.ok) {
    ...
    .then((data) => {
      if (data.type === "categories-grid" && data.data) {
...
document.addEventListener("DOMContentLoaded", function () {
  fetch("http://localhost:3000/data-12")
    ...
    .catch((error) => console.error("Error fetching magazine data:", error));
});
